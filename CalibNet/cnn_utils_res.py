...
def weight_variable_fc_small(shape, str):
    '''
    Helper function to create a weight variable initialized with
    a normal distribution (truncated to two standard devs)
    Parameters
    ...
    # W = 0.02*tf.get_variable("weight" + str, shape=shape, initializer=tf.truncated_normal_initializer(stddev=0.01))
    W = 0.015*tf.get_variable("weight" + str, shape=shape, initializer=tf.contrib.layers.xavier_initializer())
    #W = tf.get_variable("weight_%d"%layerno, shape=shape, initializer=tf.zeros_initializer)
...
def conv2d_batchnorm_reverse(x, W, name, phase, stride = [1,1,1,1], relu = True):
