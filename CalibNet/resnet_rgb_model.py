    ...
    def layer(self, layer_input, layer_no):
        ...
        for b_no in range(1,3):
            ...
            if(layer_no > 1 and b_no == 1):
                ...
                W_dn = np.array(layer_dict['block_1']['conv1']['weight'], dtype = np.float32)
                bn_mov_mean_dn = np.array(layer_dict['block_1']['bn1']['running_mean'], dtype = np.float32)
                bn_mov_var_dn = np.array(layer_dict['block_1']['bn1']['running_var'], dtype = np.float32)
                bn_gamma_dn = np.array(layer_dict['block_1']['bn1']['weight'], dtype = np.float32)
                bn_beta_dn = np.array(layer_dict['block_1']['bn1']['bias'], dtype = np.float32)
